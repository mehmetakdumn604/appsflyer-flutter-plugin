      ...
      try {
        ...
        switch (callMap["id"]) {
          ...
          case "generateInviteLinkSuccess":
            ...
            Map fullResponse = {"status": callMap['status'], "payload": decodedData};
            _callbacksById[callMap["id"]]!(fullResponse);
            break;
          case "onDeepLinking":
            Error? error = (callMap["deepLinkError"] as String?)?.errorFromString();
            Status? status = (callMap["deepLinkStatus"] as String?)?.statusFromString() ?? Status.PARSE_ERROR;
            Map<String, dynamic>? map = callMap["deepLinkObj"] as Map<String, dynamic>?;
...
Future<CancelListening?> startListeningToUDL(UDLCallback callback, String callbackName) async {
  if (Platform.isAndroid) return null;
  _channel.setMethodCallHandler(_methodCallHandler);
